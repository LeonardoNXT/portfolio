...
class ImageTrailVariant2 {
  ...
  constructor(container: HTMLDivElement, parentEl: HTMLElement) {
    ...
    this.parentEl.addEventListener("mousemove", handlePointerMove);
    this.parentEl.addEventListener("touchmove", handlePointerMove);
    ...
    this.parentEl.addEventListener("mousemove", initRender as EventListener);
    this.parentEl.addEventListener("touchmove", initRender as EventListener);
  }
...
export default function ImageTrail({
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      ...
      new ImageTrailVariant2(containerRef.current, parentRef.current);
    }, 100);
  ...
  return (
    <div
      className="absolute inset-0 z-20 pointer-events-none overflow-hidden saturate-0"
      ref={containerRef}
    >
      {items.map((url, i) => (
        <div
          className="content__img w-[190px] md:w-[250px] aspect-[1.1] rounded-[15px] absolute top-0 left-0 saturate-50 sepia-1/2 opacity-0 overflow-hidden [will-change:transform,filter]"
          key={i}
        >
          <div
